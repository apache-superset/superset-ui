...
const CALLBACK_VIZ_TYPES = ['bar', 'line', 'box_plot', 'dist_bar', 'pie'];

const NOOP = () => {};
const formatter = getNumberFormatter();

export function drawGraph(element, props, onComplete) {
  const {
    data,
    width: maxWidth,
    height: maxHeight,
  ...
  d3Element.style('opacity', onComplete ? 0 : 1);
  ...
  if (onComplete) {
    if (chart.dispatch && chart.dispatch.renderEnd && CALLBACK_VIZ_TYPES.includes(vizType)) {
      chart.dispatch.on('renderEnd', () => {
        onComplete(d3Element);
      });
    } else {
      onComplete(d3Element);
    }
