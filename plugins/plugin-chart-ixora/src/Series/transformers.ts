...
import { EchartsSeriesType, SeriesDataRecord } from './types';
...
export function getPadding(
  ...
  const yAxisOffset = addYAxisLabelOffset ? SERIES_CONSTANTS.yAxisLabelTopOffset : 0;
  return getChartPadding(showLegend, legendOrientation, margin, {
    top: SERIES_CONSTANTS.gridOffsetTop + yAxisOffset,
    bottom: zoomable
      ? SERIES_CONSTANTS.gridOffsetBottomZoomable
      : SERIES_CONSTANTS.gridOffsetBottom,
    left: SERIES_CONSTANTS.gridOffsetLeft,
    right:
      showLegend && legendOrientation === LegendOrientation.Right
        ? 0
        : SERIES_CONSTANTS.gridOffsetRight,
  });
}
