...
import { NULL_STRING, SERIES_CONSTANTS } from '../constants';
import { LegendOrientation, LegendType } from '../types';
import { defaultLegendPadding } from '../defaults';
import { SeriesDataRecord } from '../Series/types';
...
export function extractSeries(
  ...
  opts: { fillNeighborValue?: number; series: string } = { series: '' },
): SeriesOption[] {
  const { fillNeighborValue } = opts;
  if (data.length === 0) return [];

  return Object.keys(data[0])
    .filter(key => key !== opts.series)
    .map(key => ({
      id: key,
      name: key,
      data: data.map((row, idx) => {
        const isNextToDefinedValue =
          isDefined(data[idx - 1]?.[key]) || isDefined(data[idx + 1]?.[key]);
        return [
          row[opts.series],
...
export function getLegendProps(
  ...
  switch (orientation) {
    ...
    case LegendOrientation.Right:
      legend.right = 0;
      legend.top = zoomable ? SERIES_CONSTANTS.legendRightTopOffset : 0;
      break;
    ...
    default:
      legend.top = 0;
      legend.right = zoomable ? SERIES_CONSTANTS.legendTopRightOffset : 0;
      break;
  }
  return legend;
}
