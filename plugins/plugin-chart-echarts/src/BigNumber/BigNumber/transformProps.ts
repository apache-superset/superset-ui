...
export default function transformProps(chartProps: BigNumberChartProps) {
  ...
  const headerFormatter =
    coltype === GenericDataType.TEMPORAL || forceTimestampFormatting
      ? formatTime
      : getNumberFormatter(yAxisFormat || metricEntry.d3format);
  ...
  const echartOptions: EChartsCoreOption = trendLineData && {
    ...
    tooltip: {
      ...
      trigger: 'axis',
      confine: true,
      formatter: (params: { data: [number, number | null] }[]) =>
        `
                  ...
                  <strong>
                    ${
                      params[0].data[1] === null
                        ? t('N/A')
                        : headerFormatter(params[0].data[1])
                    }
                  </strong>
                `,
    },
