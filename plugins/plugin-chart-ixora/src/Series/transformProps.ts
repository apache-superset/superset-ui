...
import {
  ...
  SeriesChartTransformedProps,
} from './types';
import { ForecastSeriesEnum, ProphetValue } from '../types';
import { parseYAxisBound } from '../utils/controls';
import { dedupSeries, extractSeries, getLegendProps } from '../utils/series';
...
import { SERIES_CONSTANTS } from '../constants';

export default function transformProps(
  chartProps: EchartsSeriesChartProps,
): SeriesChartTransformedProps {
  ...
  const {
    ...
    emitFilter,
    groupby,
  }: EchartsSeriesFormData = { ...DEFAULT_FORM_DATA, ...formData };
  ...
  const echartOptions: EChartsOption = {
    ...
    toolbox: {
      show: zoomable,
      top: SERIES_CONSTANTS.toolboxTop,
      right: SERIES_CONSTANTS.toolboxRight,
      feature: {
        dataZoom: {
          yAxisIndex: false,
          title: {
    ...
    dataZoom: zoomable
      ? [
          {
            type: 'slider',
            start: SERIES_CONSTANTS.dataZoomStart,
            end: SERIES_CONSTANTS.dataZoomEnd,
            bottom: SERIES_CONSTANTS.zoomBottom,
          },
        ]
      : [],
  };
